...
import LowCarb from '../component/FilterSearch/Low-carb'
  ...
  return (
      ...
      <div className="LowApp">
       <div className="containerApp">
         <div className="col">
           <h1>Low-Carb Food</h1>
           <h2>Love low carbohydrate DIET</h2>
           <form className="search-form" onSubmit={onSubmit}>
             {alert !== "" && <Alert alert={alert}/>}
             <input type="text" placeholder="Wakanda Food U Lookin' For?" autoComplete="off" onChange={onChange} value={query}/>
             <input type="submit" value="Low Carb Only"/>
           </form>
         </div>
         <div className="recipes">
           {recipes !== [] && recipes.map(recipe => <LowCarb key={uuidv4()}  recipe= {recipe}/>)}
         </div>
       </div>
     </div>
